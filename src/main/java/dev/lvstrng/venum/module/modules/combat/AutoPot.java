...
public final class AutoPot extends Module implements TickListener {
	...
	private final NumberSetting potAmount = new NumberSetting(EncryptedString.of("Pot Amount"), 1, 10, 1, 1);
	...
	private int switchClock, throwClock, prevSlot, potsThrown;
	...
	public AutoPot() {
		...
		addSettings(minHealth, potAmount, switchDelay, throwDelay, goToPrevSlot, lookDown);
	}
	...
	public void onTick() {
		...
		if ((mc.player.getHealth() <= minHealth.getValueFloat() || bool)) {
			...
			// Stop throwing once we've thrown the desired number of pots
			if (potsThrown >= potAmount.getValue()) {
				if (prevSlot != -1 || prevPitch != -1) {
					if (prevSlot != -1) {
