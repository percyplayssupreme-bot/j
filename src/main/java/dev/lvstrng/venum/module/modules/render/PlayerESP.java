...
import dev.lvstrng.venum.utils.ProjectionUtils;
...
public final class PlayerESP extends Module implements GameRenderListener, HudListener {
	...
	private Box getRenderBox(PlayerEntity player, float tickDelta) {
		...
		double dx = lerpedPos.x - player.getX();
		double dy = lerpedPos.y - player.getY();
		double dz = lerpedPos.z - player.getZ();
		return player.getBoundingBox().offset(dx, dy, dz);
	}
